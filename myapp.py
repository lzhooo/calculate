...
def caculate(expression):
    ...
    for i in expression:
        if i in data :
            ...
            try:
                if i =='+':
                    num.append(Fraction(num2)+Fraction(num1))
                if i =='-':
                    num.append(Fraction(num2)-Fraction(num1))
                if i =='*':
                    num.append(Fraction(num2)*Fraction(num1))
...
def CalculateNum(num):
    ...
    while i <= int(num):
        ...
        if a not in f1.readlines():
            nbl = RemoveRe(a)
            answer=caculate(nbl)
            if answer =='null' or '-' in str(answer):
                pass
            else:
...
def CalculateCheck(file_a,file_c):
    ...
    f4=open('grade.txt','w')
    f4.write('总共答对'+str(len(right_list))+'题\n'+'正确的是:'+str(right_list)+'\n错误的是:'+str(wrong_list))
...
def main():
    if sys.argv[1] =='-n':
        ...
        CalculateCheck( sys.argv[2], sys.argv[4])
    else:
        print("输入格式有误")
    sys.exit(1)
